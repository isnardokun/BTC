...
class Store:
    ...
    def upsert_candles(self, symbol: str, interval: str, df: pl.DataFrame) -> int:
        if df.is_empty():
            return 0
        tmp = df.with_columns(pl.lit(symbol.upper()).alias("symbol"), pl.lit(interval).alias("interval")).select(
    ...
    def candles(self, symbol: str, interval: str) -> pl.DataFrame:
        ...
        """, [symbol.upper(), interval]).pl()

    def last_ts(self, symbol: str, interval: str) -> int | None:
        row = self.con.execute("SELECT max(ts) FROM candles WHERE symbol = ? AND interval = ?", [symbol.upper(), interval]).fetchone()
