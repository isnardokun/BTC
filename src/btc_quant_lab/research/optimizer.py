...
def optimize(df: pl.DataFrame, min_trades: int = 20) -> list[dict]:
    rows = []
    for motor, range_mode, min_bars, max_pending in product(["M1", "M3"], ["R4", "R7", "R8"], [2, 3, 4, 5], [0, 3, 5, 8]):
        cfg = PivotConfig(motor=motor, range_mode=range_mode, min_bars=min_bars, max_pending=max_pending)
        sigs = detect_pivots(df, cfg)
        _, metrics = reversal_backtest(sigs)
        rows.append({"config": {"motor": motor, "range_mode": range_mode, "min_bars": min_bars, "max_pending": max_pending}, **metrics, "score": trading_score(metrics, min_trades), "signals": len(sigs)})
